...
On Foo, we are not going to use `utils.match` instead we will use `utils.check`, match returns an object with all those state things, you can use utils.check, it returns a boolean, if the only thing you need is to check if the url matches a definition (that is our case!)
